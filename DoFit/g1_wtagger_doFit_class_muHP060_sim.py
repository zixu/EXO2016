...
parser.add_option('-a', '--additioninformation',action="store",type="string",dest="additioninformation",default="EXO_25ns_060")
...
class doFit_wj_and_wlvj:
    ...
    def __init__(self, in_channel,in_signal_sample, in_mj_min=40, in_mj_max=130, label="", input_workspace=None):
        ...
        #self.file_Directory="TTBarDataSet/MINIAODv2_work/";
        self.file_Directory="TTBarDataSet/MINIAODv2_work/060_2p1/";
        ...
        if self.wtagger_label=="HP" :
            if self.channel=="el":self.wtagger_cut=0.60 ; self.wtagger_cut_min=0. ;
            if self.channel=="mu":self.wtagger_cut=0.60 ; self.wtagger_cut_min=0. ;

        if self.wtagger_label=="HP" :
         if self.channel=="el":self.wtagger_cut=0.60 ; self.wtagger_cut_min=0. ;
         if self.channel=="mu":self.wtagger_cut=0.60 ; self.wtagger_cut_min=0. ;
         if self.wtagger_label=="LP": self.wtagger_cut=0.75 ; self.wtagger_cut_min=0.60 ;
   ...
   #### Method to make a RooAbsPdf giving label, model name, spectrum, if it is mc or not and a constraint list for the parameters
    ...
    def make_Pdf(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[]):
        ...
        if in_model_name == "ErfExpGaus":
            ...
            rrv_mean_gaus  = RooRealVar("rrv_mean_gaus"+label+"_"+self.channel,"rrv_mean_gaus"+label+"_"+self.channel,82,65,95);
        ...
        if in_model_name == "ErfExpGaus_sp":#offset == mean
            ...
            rrv_mean1_gaus   = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel,"rrv_mean1_gaus"+label+"_"+self.channel,80,65,95);
        ...
        if in_model_name == "ExpGaus":
            ...
            rrv_mean1_gaus  = RooRealVar("rrv_mean1_gaus"+label+"_"+self.channel,"rrv_mean1_gaus"+label+"_"+self.channel,84,55,100);
...
#       qun add
        if in_model_name == "GausExp_ttbar_failtau2tau1cut":
          ...
          model_pdf = RooAddPdf("model_pdf"+label+"_"+self.channel+mass_spectrum,"model_pdf"+label+"_"+self.channel+mass_spectrum,RooArgList(gaus,exp),RooArgList(rrv_high),1)
...
## all the parameters contrained within the assigned error
	...
	if self.wtagger_label=="HP":
            model_ttbar_data_fail = self.make_Model_for_ttbar_controlsample("_ttbar_data"+label+"_"+"failtau2tau1cut","GausExp_ttbar_failtau2tau1cut","_mj",self.constrainslist_data,label); 
            #model_ttbar_data_fail = self.make_Model_for_ttbar_controlsample("_ttbar_data"+label+"_"+"failtau2tau1cut","GausChebychev_ttbar_failtau2tau1cut","_mj",self.constrainslist_data,label); 
	else :
...
#         simPdf_TotalMC.plotOn(xframe_data_fail,RooFit.Name("MC fit"),RooFit.Slice(category_p_f,"fail"),RooFit.ProjWData(RooArgSet(category_p_f),combData_p_f_TotalMC),RooFit.NormRange("controlsample_fitting_range"), RooFit.LineStyle(kDashed))
        ...
        #signal window
        lowerLine = TLine(self.mj_signal_min,0.,self.mj_signal_min,0.75*xframe_data.GetMaximum()); lowerLine.SetLineWidth(2); lowerLine.SetLineColor(kGray+2); lowerLine.SetLineStyle(9);
        upperLine = TLine(self.mj_signal_max,0.,self.mj_signal_max,0.75*xframe_data.GetMaximum()); upperLine.SetLineWidth(2); upperLine.SetLineColor(kGray+2); upperLine.SetLineStyle(9);
        #xframe_data.addObject(lowerLine); xframe_data.addObject(upperLine);
        lowerLine = TLine(self.mj_signal_min,0.,self.mj_signal_min,0.75*xframe_data_fail.GetMaximum()); lowerLine.SetLineWidth(2); lowerLine.SetLineColor(kGray+2); lowerLine.SetLineStyle(9);
        upperLine = TLine(self.mj_signal_max,0.,self.mj_signal_max,0.75*xframe_data_fail.GetMaximum()); upperLine.SetLineWidth(2); upperLine.SetLineColor(kGray+2); upperLine.SetLineStyle(9);
...
#         simPdf_TotalMC.plotOn(xframe_data_fail,RooFit.Name("MC fit"),RooFit.Slice(category_p_f,"fail"),RooFit.ProjWData(RooArgSet(category_p_f),combData_p_f_TotalMC),RooFit.NormRange("controlsample_fitting_range"), RooFit.LineStyle(kDashed))
    ...
    def banner4Plot(self, iswithpull=0):
      ...
      if iswithpull:
       if self.channel=="el":
        banner = TLatex(0.3,0.96,("CMS Preliminary, %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
       elif self.channel=="mu":
        banner = TLatex(0.3,0.96,("CMS Preliminary, %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
       banner.SetNDC(); banner.SetTextSize(0.04);
      else:
       if self.channel=="el":
        banner = TLatex(0.22,0.96,("CMS Preliminary, %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
       if self.channel=="mu":
        banner = TLatex(0.22,0.96,("CMS Preliminary, %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
    ...
    def legend4Plot(self, plot, left=1, isFill=1, xoffset=0., yoffset=0., x_right_offset=0., y_upper_offset=0.):
        ...
        if left==-1:
            ...
            theLeg = TLegend(0.02+xoffset, 0.68+yoffset, 0.8+xoffset+x_right_offset, 0.95+yoffset+y_upper_offset, "", "NDC");
            theLeg.SetFillColor(0);
            theLeg.SetFillStyle(0);
    ...
    def fit_TTbar_controlsample(self, isherwig=0):
      ...
      if isherwig ==0:
        ...
        if self.channel=="mu":
         #self.fit_mj_single_MC(self.file_STop_mc,"_STop","ErfExpGaus","_TTbar_controlsample"); ## Erf*exp + Gaus pass sample
	 ...
	 self.fit_mj_single_MC(self.file_STop_mc,"_STop","ExpGaus","_TTbar_controlsample");
         ...
         #self.fit_mj_single_MC(self.file_STop_mc,"_STop","ErfExpGaus_sp","_TTbar_controlsample"); ## Exp + Gaus pass sample
        ...
        self.ScaleFactor_forPureWJet_TTbar_controlsample(self.file_data);
          ...
          if self.channel=="mu":
           self.fit_mj_single_MC(self.file_STop_mc,"_STop_herwig","ErfExpGaus","_TTbar_controlsample");
          ...
          self.fit_mj_single_MC(self.file_STop_mc,"_STop_herwig_failtau2tau1cut","ErfExpGaus","_TTbar_controlsample");
          ...
          self.fit_mj_single_MC(self.file_WJets1_mc,"_WJets0_herwig_failtau2tau1cut","ErfExp","_TTbar_controlsample");
...
class doFit_wj_and_wlvj_simultaneous:
    def __init__(self,isherwig=0):
        ...
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger eff of data el %s     : %0.3f +/- %0.3f"%(label,rrv_eff_data_el.getVal(),rrv_eff_data_el.getError()) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger eff of MC el %s     : %0.3f +/- %0.3f"%(label,rrv_eff_MC_el.getVal(),rrv_eff_MC_el.getError()) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger eff of data mu %s        : %0.3f +/-%0.3f"%(label,rrv_eff_data_mu.getVal(),rrv_eff_data_mu.getError()) )
        self.boostedW_fitter_el.file_out_ttbar_control.write( "\nPure W-tagger eff of MC mu %s        : %0.3f +/-%0.3f"%(label,rrv_eff_MC_mu.getVal(),rrv_eff_MC_mu.getError()) )
        ...
        tmpq_eff_MC_mu_LP =1. - rrv_eff_MC_mu.getVal() ;
        tmpq_eff_data_el_LP =1. - rrv_eff_data_el.getVal() ;
        tmpq_eff_data_mu_LP =1. - rrv_eff_data_mu.getVal() ;

        tmpq_eff_MC_el_LP_err = TMath.Sqrt( rrv_eff_MC_el.getError()**2);# + tmpq_eff_MC_el_extremefail_error**2 );
        tmpq_eff_MC_mu_LP_err = TMath.Sqrt( rrv_eff_MC_mu.getError()**2 );#+ tmpq_eff_MC_mu_extremefail_error**2 );
        tmpq_eff_data_el_LP_err = TMath.Sqrt( rrv_eff_data_el.getError()**2);# + tmpq_eff_data_el_extremefail_error**2 );
        tmpq_eff_data_mu_LP_err = TMath.Sqrt( rrv_eff_data_mu.getError()**2);# + tmpq_eff_data_mu_extremefail_error**2 );
        ...
        self.boostedW_fitter_el.file_out_ttbar_control.write("\nLP QEff of el data %s: %0.3f +/- %0.3f"%(label,tmpq_eff_data_el_LP, tmpq_eff_data_el_LP_err));
        self.boostedW_fitter_el.file_out_ttbar_control.write("\nLP QEff of el MC %s: %0.3f +/- %0.3f"%(label,tmpq_eff_MC_el_LP, tmpq_eff_MC_el_LP_err));
        self.boostedW_fitter_mu.file_out_ttbar_control.write("\nLP QEff of mu data %s: %0.3f +/- %0.3f"%(label,tmpq_eff_data_mu_LP, tmpq_eff_data_mu_LP_err));
        self.boostedW_fitter_mu.file_out_ttbar_control.write("\nLP QEff of mu MC %s: %0.3f +/- %0.3f"%(label,tmpq_eff_MC_mu_LP, tmpq_eff_MC_mu_LP_err));
        self.boostedW_fitter_el.file_out_ttbar_control.write("\nLP  of el MC %s: %0.3f +/- %0.3f"%(label,rrv_number_ttbar_TotalMC_extremefailtau2tau1cut_el_mj.getVal(), rrv_number_ttbar_data_extremefailtau2tau1cut_el_mj.getVal()));
        ...
        pureq_wtagger_sf_mu_LP = tmpq_eff_data_mu_LP / tmpq_eff_MC_mu_LP;
        ...
        pureq_wtagger_sf_mu_LP_err = pureq_wtagger_sf_mu_LP*TMath.Sqrt( (tmpq_eff_data_mu_LP_err/tmpq_eff_data_mu_LP)**2 + (tmpq_eff_MC_mu_LP_err/tmpq_eff_MC_mu_LP)**2 );
        ...
        self.boostedW_fitter_el.file_out_ttbar_control.write("\nPure W-tagger LP SF of el %s: %0.3f +/- %0.3f"%(label,pureq_wtagger_sf_el_LP, pureq_wtagger_sf_el_LP_err));
        self.boostedW_fitter_mu.file_out_ttbar_control.write("\nPure W-tagger LP SF of mu %s: %0.3f +/- %0.3f"%(label,pureq_wtagger_sf_mu_LP, pureq_wtagger_sf_mu_LP_err));
